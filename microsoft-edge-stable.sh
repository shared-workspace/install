#!/bin/sh
# This script installs Microsoft Edge using the OS's package manager
...
main() {
    ...
    case "$os" in
        Darwin) error "This script does not support macOS. Please download Microsoft Edge from https://www.microsoft.com/edge";;
    ...
    if available apt-get; then
        export DEBIAN_FRONTEND=noninteractive
        show $curl https://packages.microsoft.com/keys/microsoft.asc | show $sudo apt-key add -
        show $sudo sh -c 'echo "deb [arch=amd64] https://packages.microsoft.com/repos/edge stable main" >> /etc/apt/sources.list.d/microsoft-edge.list'
        show $sudo apt-get update
        show $sudo apt-get install -y microsoft-edge-stable

    elif available dnf; then
        show $sudo dnf install -y 'dnf-command(config-manager)'
        show $sudo dnf config-manager --add-repo=https://packages.microsoft.com/yumrepos/edge
        show $sudo dnf install -y microsoft-edge-stable

    elif available yum; then
        available yum-config-manager || show $sudo yum install yum-utils -y
        show $sudo yum-config-manager --add-repo=https://packages.microsoft.com/yumrepos/edge
        show $sudo yum install -y microsoft-edge-stable

    elif available zypper; then
        show $sudo zypper --non-interactive addrepo --gpgcheck --repo https://packages.microsoft.com/yumrepos/edge
        show $sudo zypper --non-interactive --gpg-auto-import-keys refresh
        show $sudo zypper --non-interactive install microsoft-edge-stable
        ...
        error "Could not find a supported package manager. Only apt, dnf, yum and zypper are supported." "" \
            ...
            "https://github.com/microsoft/edge/issues and include the following information:" "" \
            "$(uname -srvmo)" "" \
            "$(cat /etc/os-release || true)"
    fi

    printf "Installation complete! Start Microsoft Edge by typing: microsoft-edge\n"
}
